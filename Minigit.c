...
Commit* findCommit(const char *hash) {
    ...
    while (curr) {
        if (strcmp(curr->hash, hash) == 0) return curr;
        curr = curr->next;
    }
    return NULL;
}
...
Commit* createCommit(const char *msg, Commit **parents, int parent_count, FileVersion *files) {
    ...
    for (int i = 0; i < parent_count; i++) c->parents[i] = parents[i];
...
void mergeBranches(const char *name1, const char *name2, const char *msg) {
    ...
    while (b1 && strcmp(b1->name, name1) != 0) b1 = b1->next;
    while (b2 && strcmp(b2->name, name2) != 0) b2 = b2->next;
    if (!b1 || !b2) { printf("❌ Branch not found.\n"); return; }
...
void logCommits(Commit *c) {
    ...
    showFiles(c->files);
    printf("-------------------------------\n");
    if (c->parent_count > 0) logCommits(c->parents[0]);
}
...
void saveCommitsToFile() {
    ...
    fclose(fp);
    printf("💾 Commits saved to commits.txt\n");
}

/* ===================== INIT REPO ===================== */
void initRepo() {
    for (int i=0;i<HASH_SIZE;i++) commitTable[i]=NULL;
